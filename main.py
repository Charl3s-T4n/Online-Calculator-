...
def calculator():            # Create calculator function ( so i can use RECURSION )
  ...
  while not flag_variable_first:      # while True:
    ...
    if continue_calculating == 'y':
      first_number = answer                    # first number will be the answer from the previous equation, while n2 will be the next number user will input 
      flag_variable_first = False      # While loop continue 
    elif continue_calculating == 'n':      # Will restart calculator 
      flag_variable_first = True       # Stop while loop
      clear()                     # Clear output in console 
      calculator()           # Call back the function which restarts 
